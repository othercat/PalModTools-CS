...
public unsafe static class Script
{
    ...
    {
        using MkfWriter coreFile = new(Config.PalWorkPath.DataBase.Core);
    ...
    public static void Compile()
    {
        ...
        File.AppendAllText(pathName, $"@{0:X4}: {0:X4} {0:X4} {0:X4} {0:X4} : ==================================================\n");
        ...
        for (i = 1; i <= end; i++)
        {
                ...
                foreach (var info in asm)
                {
                    ...
                    switch (info.Type)
                    {
                        ...
                        case FuncInformation.SpecialType.HeroEntity:
                            Value(Config.GetNewEntityId(EntityType.Hero, GetArgVal(argId)));
                            break;

                        case FuncInformation.SpecialType.ItemEntity:
                            Value(Config.GetNewEntityId(EntityType.Item, GetArgVal(argId)));
                            break;

                        case FuncInformation.SpecialType.MagicEntity:
                            Value(Config.GetNewMagicId((short)GetArgVal(argId)));
                            break;

                        case FuncInformation.SpecialType.EnemyEntity:
                            Value(Config.GetNewEntityId(EntityType.Enemy, GetArgVal(argId)));
                            break;

                        case FuncInformation.SpecialType.PoisonEntity:
                            Value(Config.GetNewEntityId(EntityType.Poison, GetArgVal(argId)));
                            break;
            ...
            short Arg(int argId) => (short)CScript->Args[argId];

            switch (CScript->Command)
            {
                case 0xFFFF:
                    typeName = $"显示对话   >{Message.GetDialogue(Arg(0))}";
                    break;
                ...
                case 0x0001:
                    typeName = "暂停执行，将 调用地址 替换为 下一条命令";
                    break;

                case 0x0002:
                    typeName = $"暂停执行，将 调用地址 替换为地址 {Arg(0):X4}。{((Arg(1) != 0) ? $"累计触发 {Arg(1)} 次后，将 调用地址 替换为 下一条命令" : "")}";
                    break;

                case 0x0003:
                    typeName = $"跳转到地址 {Arg(0):X4}，完成跳转地址的执行后脚本结束。{((Arg(1) != 0) ? $"累计触发 {Arg(1)} 次后，该指令将成为 NOP" : "")}";
                    break;

                case 0x0004:
                    typeName = $"调用地址 {Arg(0):X4}，完成调用后返回并继续执行；{((Arg(1) != 0) ? $"触发者：Event {Arg(1)}" : "")}";
                    break;

                case 0x0005:
                    typeName = $"若屏幕上有对话则会先等待按下任意键，之后再更新画面，{((Arg(2)) != 0 ? "" : "不")}更新队伍步伐，更新后延迟 {Arg(1) * 60} ms";
                    break;

                case 0x0015:
                    typeName = $"将 {Arg(2)} 的方向设置为 {Arg(0)}，帧号设置为 {Arg(1)}";
                    break;

                case 0x001F:
                    typeName = $"将道具 {Arg(0)} 添加到背包中，数量 {Arg(1)}";
                    break;

                case 0x0020:
                    typeName = $"将道具 {Arg(0)} 在背包中删除，数量 {((Arg(1) != 0) ? Arg(1) : 1)}，背包中道具不足，则会从角色身上卸下；";
                    break;
                ...
                case 0x003E:
                    typeName = "设置对话框位置，中间条幅";
                    break;

                case 0x0058:
                    typeName = $"如果库存中的道具 {Arg(0)} 少于 {Arg(1)} 个，则跳转到 {Arg(2):X4}";
                    break;

                case 0x0086:
                    typeName = $"若所有队员中身上装备的 {Arg(0)} 数量不足 {Arg(1)}，则跳转到 {Arg(2):X4}";
                    break;
                ...
                case 0x00A7:
                    typeName = "设置对话框位置，Item/Magic 描述";
                    break;

                default:
                    typeName = $"{(short)CScript->Args[0]} {(short)CScript->Args[1]} {(short)CScript->Args[2]}";
                    break;
            }
            File.AppendAllText(pathName, $"@{i:X4}: {CScript->Command:X4} {CScript->Args[0]:X4} {CScript->Args[1]:X4} {CScript->Args[2]:X4} : {typeName}\n");
        }
    }
}
