...
格式显式选择：`yj1` 为 YJ_1、`yj2` 为 Win 压缩、`none` 为未压缩；不通过文件夹名称推测 DOS/Win。`sprite` 读取16位字偏移帧组，`rle` 读取单张 RLE，`bitmap320` 要求恰好64000字节背景。`family` 是作者选择的来源标签，支持 ABC/F/MGO/FIRE/RGM/FBP/DATA，不等于这些文件的每种变体都已支持。帧组支持无尾标记、指向数据末尾的偏移，以及 DATA#9 可见的单个尾部零偏移；零尾项前的最后一张仍读到 chunk 末尾。内部零、重复/逆序、越界、双尾标记和扩展大帧表仍报错，不采用血蛊特殊指针截断规则。
...
可选 `--frames 18,39,40` 只解码明确选择的帧，输出仍按原帧号命名并排序，例如选择40产生 `00040.png`，不会变为00000。整个偏移表始终验证；未选帧的RLE内容没有被验证。重复、负数、范围简写与不存在的帧会拒绝。省略时保持整组导出。

可选 `--color-bank 0..15` 与 `--shade-shift -15..15` 用于明确复现索引调色：先将低四位加偏移并夹在0–15，再换高四位色组，最后查PAT并做RGB6×4；不改变alpha。色组是序号1（对应0x10），不是输入16或0x10；省略shift为0，单独给shift会拒绝。省略两项保持旧PNG字节。例：命令普通态追加 `--frames 40,41,42,43 --color-bank 0 --shade-shift -4`，禁用态使用bank1；选中态不做索引变换。该映射来自具名梦时空 `uibattle.c` / `palcommon.c`，不代表所有改版的规则或配色。

`export.json` 保留源文件/chunk/解压字节/所选调色板/输出PNG与RGBA哈希，另记实际解析程序集哈希、源帧总数/所选帧及索引变换顺序/参数。它是本工具的导出凭据，尚非跨仓 Native 合同，工坊不会自动采信。动作、对象身份、谱系、时长与分发权限须单独审查；文件编号不自动等于对象号。
