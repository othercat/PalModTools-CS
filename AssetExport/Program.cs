...
public static class Program
{
    ...
    private static int Run(string[] args)
    {
        ...
        {
            Console.WriteLine("PalAssetExport --mkf FILE --chunk N --kind sprite|rle|bitmap320 --compression yj1|yj2|none --family ABC|F|MGO|FIRE|RGM|FBP|DATA --palette PAT.MKF --palette-chunk N --variant day|night --output NEW-DIRECTORY [--frames 18,39,40] [--color-bank 0..15 [--shade-shift -15..15]]\nExports original-size RGBA PNGs and source hashes. Never writes source resources. No game initialization, Python, C++ library or network."); return 0;
        }
        var options = new Dictionary<string,string>(StringComparer.Ordinal);
        var required = new[] { "--mkf", "--chunk", "--kind", "--compression", "--family", "--palette", "--palette-chunk", "--variant", "--output" };
        var allowed = required.Concat(new[] { "--frames", "--color-bank", "--shade-shift" }).ToArray();
        ...
        foreach (var name in required) if (!options.ContainsKey(name)) throw new FormatException("Missing " + name);
        var selection = FrameSelection.Parse(options.GetValueOrDefault("--frames"));
        var transform = PaletteTransform.Parse(options.GetValueOrDefault("--color-bank"), options.GetValueOrDefault("--shade-shift"));
        ...
        var selectedFrames = FrameSelection.Resolve(selection, frames.Count);
        var images = new List<RlePixels>(); var remainingPixels = 33554432;
        foreach (var frame in selectedFrames)
        {
            var bytes = frames[frame];
        ...
        {
            var image = images[i]; var rgba = transform.ToRgba(image, rgb6); var frame = selectedFrames[i];
            var name = frame.ToString("D5") + ".png";
            PngFile.Write(Path.Combine(staging, name), image.Width, image.Height, rgba);
            records.Add(new { frame, file = name, width = image.Width, height = image.Height, sha256 = ResourceInput.FileHash(Path.Combine(staging, name)), rgba_sha256 = ResourceInput.Hash(rgba), rle_consumed_bytes = image.ConsumedBytes,
        ...
        var receipt = new { schema = "palmod.image-export.v1", family, kind, compression = options["--compression"],
            ...
            indexed_transform = new { mode = transform.ColorBank is null ? "identity" : "replace-high-nibble-clamp-low-nibble", color_bank = transform.ColorBank, shade_shift = transform.ShadeShift, order = "source-index; indexed-transform; PAT-lookup; rgb6-times-four" },
            selection = new { mode = selection is null ? "all" : "explicit-source-frames", source_frame_count = frames.Count, source_frames = selectedFrames },
