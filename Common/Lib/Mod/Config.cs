...
public static unsafe class Config
{
    ...
    public static Dictionary<Entity.Type, int> NewEntityRange { get; set; } = [];
    ...
    public static void AddNewEntityRange(Entity.Type type, int beginId) =>
        NewEntityRange[type] = beginId;
    ...
    public static int GetNewEntityId(Entity.Type type, int entityId)
    {
        S.Failed(
            "Config.GetNewEntityId",
            $"The entity type '{type}' is undefined",
            NewEntityRange.TryGetValue(type, out var beginId)
        );

        return beginId + entityId;
    }
}
